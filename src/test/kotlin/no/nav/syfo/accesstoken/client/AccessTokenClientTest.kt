...
class AccessTokenClientTest : Spek({
    ...
    val mockServer = embeddedServer(Netty, mockHttpServerPort) {
        ...
        routing {
            get("/aad") {
                when {
                    call.request.headers["resource"] == "resource1" -> call.respond(AadAccessToken("token1", Instant.now().plusSeconds(200)))
                    call.request.headers["resource"] == "resource2" -> call.respond(HttpStatusCode.Unauthorized, "Noe gikk galt")
    ...
    val accessTokenClient = AccessTokenClient("$mockHttpServerUrl/aad", "1", "2", httpClient)
        ...
        it("Should get 401 when inncorrect secret") {
            runBlocking {
                assertFailsWith<ClientRequestException>() {
                    accessTokenClient.hentAccessToken("resource2")
                }
            }
        }
    }
