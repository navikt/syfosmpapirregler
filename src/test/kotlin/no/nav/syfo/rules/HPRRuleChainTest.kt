...
class HPRRuleChainTest : Spek({
    describe("1402 Behandler er ikke gyldig i HPR på konsultasjonstidspunktet") {
        it("Should not trigger rule") {
            HPRRuleChain.BEHANDLER_IKKE_GYLDIG_I_HPR(RuleData(generateSykemelding(generatePerioder()),
                getGyldigBehandler())) shouldEqual false
        }

        it("Should not trigger rule") {
            HPRRuleChain.BEHANDLER_IKKE_GYLDIG_I_HPR(getRuleData(getUgyldigBehandler())) shouldEqual true
        }
    }
    ...
    describe("1403 Behandler har ikke gyldig autorisasjon i HPR") {
        it("Should not trigger rule for kodeverdi") {
            ...
            for (verdi in validAuthorizations) {
                ...
                HPRRuleChain.BEHANDLER_MANGLER_AUTORISASJON_I_HPR(getRuleData(behandler)) shouldEqual false
            }
        }
        it("Should trigger rule when verdi is not valid") {
            val behandler = getBehandler(autorisasjon = Kode(true, 7704, ""))
            HPRRuleChain.BEHANDLER_MANGLER_AUTORISASJON_I_HPR(getRuleData(behandler)) shouldEqual true
        }
    ...
    describe("1407 Behandler finnes i HPR men er ikke lege, kiropraktor, manuellterapeut, fysioterapeut eller tannlege") {
        it("Should not trigger rule") {
            ...
            for (type in behandlerTyper) {
                val behandler = getBehandler(helsepersonellkategori = Kode(aktiv = true, verdi = type, oid = 9060))
                HPRRuleChain.BEHANDLER_IKKE_LE_KI_MT_TL_FT_I_HPR(getRuleData(behandler)) shouldEqual false
            }
        }
        it("Should trigger rule when not valid kategori") {
            val behandler = getBehandler(helsepersonellkategori = Kode(aktiv = true, verdi = "DD", oid = 9060))
            HPRRuleChain.BEHANDLER_IKKE_LE_KI_MT_TL_FT_I_HPR(getRuleData(behandler)) shouldEqual true
        }
        it("Should trigger rule when helsepersonellkategori ikke er aktiv") {
            val behandler = getBehandler(helsepersonellkategori = Kode(aktiv = false, verdi = "LE", oid = 9060))
            HPRRuleChain.BEHANDLER_IKKE_LE_KI_MT_TL_FT_I_HPR(getRuleData(behandler)) shouldEqual true
        }
    }
})
